...
export class Sushiro {
  ...
  public random: (random_num?: number) => Promise<string> = async (
    random_num?: number
  ): Promise<string> => {
    const all_menu: string[][] = await menu();
  ...
  public price: (search_word?: string) => Promise<string> = async (
    search_word?: string
  ): Promise<string> => {
    const all_menu: string[][] = await menu();
    const title: string[] = all_menu.map((i: string[]): string => {
    ...
    if (search_word) {
      const nums: number[] = title
        .map((v: string, i: number): number => {
  ...
  public calorie: (search_word?: string) => Promise<string> = async (
    search_word?: string
  ): Promise<string> => {
    const all_menu: string[][] = await menu();
    const title: string[] = all_menu.map((i: string[]): string => {
